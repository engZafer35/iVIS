...
uint32_t cs43l22_Init(uint16_t DeviceAddr, uint16_t OutputDevice, uint8_t Volume, uint32_t AudioFreq)
{
  ...
  AUDIO_IO_Read(DeviceAddr, CS43L22_REG_POWER_CTL2);
...
#define MAX_BEEP_VOLUME	(0x1F) // volume value should be less than this max value
...
{
	uint32_t retVal = -1; // return value, -1 failure
    uint8_t regVal = 0;

	// According to RM,  Clear these bits.
	regVal |= onTime; // lower 4 bits represents beep on time.
	regVal |= frq; // higher 4 bits represents beep frequency time.

	AUDIO_IO_Write(DeviceAddr, CS43L22_REG_BEEP_FREQ_ON_TIME, regVal);


	regVal = AUDIO_IO_Read(DeviceAddr, CS43L22_REG_BEEP_VOL_OFF_TIME);
    if (-1 != regVal) // -1 error
    {
    	regVal &= 0x1F; // firstly clear off time bits, higher 3 bit
    	regVal |= offTime;
    	AUDIO_IO_Write(DeviceAddr, CS43L22_REG_BEEP_VOL_OFF_TIME, regVal);

    	retVal = 0; //function return value, 0 success
    }
...
uint32_t cs43l22_startStopBeep(uint16_t DeviceAddr, enum EN_BEEP_MODE mod)
{
    ...
    regVal = AUDIO_IO_Read(DeviceAddr, CS43L22_REG_BEEP_TONE_CFG);
    if (-1 != regVal) // -1 error
    {
		regVal &= 0x3F; // firstly clear off time bits, higher 2 bit
		regVal |= mod; // higher 4 bits represents beep on time.

		AUDIO_IO_Write(DeviceAddr, CS43L22_REG_BEEP_TONE_CFG, regVal);
		retVal = 0; //function return value, 0 success
    }
