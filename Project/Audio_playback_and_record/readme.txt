...
Click feature has been implemented only for RevD board. A click on the board
...
the recorded wave length is (~30secs) and user can interrupt the recording using the User button. 
 ...
 - Once the recording is finished or interrupted, the recorded wave will start playing
   automatically and LED6 start toggling, Plug a headphone (CN4) to hear a 16K sound  /!\ Take care of yours ears.
 - If the define "PLAY_REPEAT_DISABLED" in main.h file is not enabled the play will start in infinity loop,
   on board RevD you can pause/resume the player using the MEMS click feature (LED6 on/toggle) 
 - Default volume is 70%.

One Mems microphone MP45DT02 mounted on STM32F4-Discovery is connected
...
@par Hardware and Software environment  
  ...
  - This application has been tested with STMicroelectronics STM32F4-Discovery RevB & RevC & RevD
